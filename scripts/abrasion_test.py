...
'''
motions for the sawyer robot to perform abrasion test on the sensor.
Robot is programmed to push the finger on a table and slide it multiple times until the rubber peels off!
Majority of the code is taken from rp_go_to_cartesian_pose.py
'''
...
def main():
    ...
    parser.add_argument(
        "-p", "--position", type=float, default=[0.7, 0.0, 0.3],
        nargs='+',
    ...
    ################################################################
    ##  INITIALIZE CONSTRUCTORS AND OTHER THINGS
    ################################################################

    for i in range(15):

        args.position = [0.7, ABRADE_DIS * (i % 2), 0.3]
        args.orientation = [0.0, 0.707, 0.0, 0.707]
        ...
        try:
            ...
            ################################################################
            ##  POPULATE THE POSITION AND ORIENTATION VARIABLES
            ################################################################
